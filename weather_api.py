...
def load_weather_data(data, cur, conn):
    cur.execute("CREATE TABLE IF NOT EXISTS weather (case_id INTEGER PRIMARY KEY, country_id INTEGER, temperature REAL, wind REAL, rain REAL)")
    ...
    while count < 25 and ind < len(locs):
        ...
        cur.execute("INSERT OR IGNORE INTO weather (case_id, country_id, temperature, wind, rain) VALUES (?,?,?,?,?)", (ind, d[0], d[1]["tavg"], d[1]["wspd"], d[1]["prcp"],))
